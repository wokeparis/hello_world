...
//stl容器里可以用引用吗？ c++是不能声明或定义对引用的指针的。容器里迭代器用指针实现，如果容器接受的类型是引用，那么就需要定义生成指向引用的指针，这是不行的。
//stack和queue为什么没有迭代器：我理解的，它是从设计的目的出发，stack和queue不需要随机访问或顺序访问的功能，并不是不能，而是没有必要，底层一般使用deque实现
//虚函数表的调用方式。//知乎 https://zhuanlan.zhihu.com/p/41309205  虚函数表存在只读数据段
//stl容器里如何放一个unique_ptr,使用std::move(),被move的原指针不能再使用.也可以使用emplace_back直接构造。
//如果一个类里有一个装有unique_ptr的容器，那么必须对它的拷贝构造函数和赋值构造函数做特殊处理。
